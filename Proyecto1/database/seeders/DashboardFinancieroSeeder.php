...
class DashboardFinancieroSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $providerId = DB::table('providers')->where('nombre', 'Proveedor General')->value('id');
        if (!$providerId) {
        ...
        // Crear un cliente predeterminado si no existe
        $clientId = DB::table('clients')->where('nombre', 'Cliente General')->value('id');
        if (!$clientId) {
        ...
        foreach ($ventasPorMes as $mesIndex => $cantidad) {
            for ($i = 0; $i < $cantidad; $i++) {
                ...
                $ventaId = DB::table('sales')->insertGetId([
                    'client_id' => $clientId,
                    'user_id' => 1,
                    'numero_factura' => 'FAC-' . $anio . '-' . str_pad(($mesIndex * 100 + $i), 6, '0', STR_PAD_LEFT),
                    'subtotal' => $subtotal,
                    'impuesto' => $impuesto,
                    'total' => $total,
                    'created_at' => $fecha,
                ...
                foreach ($productosVenta as $pv) {
                    DB::table('sale_items')->insert([
                        ...
                        'subtotal' => $pv['precio'] * $pv['cantidad'],
                        'created_at' => $fecha,
                        'updated_at' => $fecha,
                    ]);
                }
        ...
        foreach ($comprasPorMes as $mesIndex => $cantidad) {
            for ($i = 0; $i < $cantidad; $i++) {
                ...
                $compraId = DB::table('purchases')->insertGetId([
                    ...
                    'user_id' => 1,
                    'numero_factura' => 'COM-' . $anio . '-' . str_pad(($mesIndex * 100 + $i + 1), 6, '0', STR_PAD_LEFT),
                    'subtotal' => $subtotal,
                    'impuesto' => $impuesto,
                    'total' => $total,
                    'created_at' => $fecha,
